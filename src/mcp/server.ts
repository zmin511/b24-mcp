...
function toolList(): ToolDef[] {
  return [
    ...
    },
    {
      ...
      risky: false,
      inputSchema: z.object({ query: z.string().min(1), limit: z.number().int().positive().optional() }).merge(baseInput),
      handler: async (ctx, input) => {
        const connectionId = input.connection_id ?? ctx.config.BITRIX_DEFAULT_CONNECTION_ID;
        ...
        const res = await users.search({ query: input.query, limit: input.limit });
        return { connectionId, res };
      }
    },
    {
    ...
    },
    {
      ...
      description: "Upload file from local path into folder. Requires confirm=true.",
      risky: true,
      inputSchema: z.object({ folder_id: z.number().int().positive(), local_path: z.string().min(1), filename: z.string().optional() }).merge(baseInput),
      handler: async (ctx, input) => {
        ...
        const disk = new BitrixDiskService(client);
        return { connectionId, res: await disk.folderUploadFile({ folderId: input.folder_id, localPath: input.local_path, filename: input.filename }) };
      }
    },
    {
